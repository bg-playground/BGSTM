...
async def generate_suggestions(
    algorithm: Optional[str] = Query(
        None,
        description="Algorithm to use: 'tfidf', 'keyword', or 'hybrid'. Uses default if not specified."
    ),
    threshold: Optional[float] = Query(
        None,
        ge=0.0,
        le=1.0,
        description="Minimum confidence threshold (0.0-1.0). Uses default if not specified."
    ),
