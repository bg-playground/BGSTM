...
async def _get_quality_evidence(db: AsyncSession, window_days: int) -> dict[str, object]:
    ...
    passed_executions = sum(
        round(point.total_executed * point.pass_rate_pct / 100) for point in pass_rate.points
    )
    overall_pass_rate = (
        round(passed_executions / total_executions * 100, 2) if total_executions else None
    )
...
def _quality_markdown(evidence: dict[str, object]) -> list[str]:
    window_days = evidence["window_days"]
    automation = evidence["automation"]
    recovery = evidence["recovery"]
    modules = evidence["modules"]
    pass_rate = evidence["pass_rate_pct"]

    lines = ["", f"## Quality KPI Evidence — Last {window_days} Days", ""]
    if pass_rate is None:
        lines.append(f"- **Execution volume and pass rate:** {evidence['pass_rate_reason'] or 'No executions are available.'}")
    else:
        lines.append(
            f"- **Execution volume and pass rate:** {evidence['total_executions']} executions; "
            f"{evidence['passed_executions']} passed ({pass_rate:.2f}%)."
        )
    lines.extend(
        [
    ...
    if automation.reason:
        ...
        lines.append(
            ...
            f"({automation.automated}/{automation.total} test cases automated)."
        )

    if recovery.mean_recovery_hours is None:
        recovery_value = recovery.reason or "No resolved recovery episodes are available."
    else:
        recovery_value = f"{recovery.mean_recovery_hours:.2f} hours"
    lines.append(
        "- **Execution Mean Time to Recovery (time-to-green):** "
        f"{recovery_value}; {recovery.resolved_episodes} resolved episode(s), "
        f"{recovery.open_episodes} still open. This is execution recovery, not defect lifecycle Mean Time To Repair."
    )

    lines.extend(["", "### Top Failing Modules", ""])
    if modules.modules:
        lines.extend(f"- {bucket.module}: {bucket.count} failed execution(s)" for bucket in modules.modules)
    else:
        lines.append(f"- {modules.reason or 'No failed executions were recorded in the selected window.'}")
    return lines


def _build_markdown(snapshot: ReadinessSnapshot, evidence: dict[str, object]) -> str:
    ...
    lines = [
        ...
        f"- **Generated at:** {generated_at}", "", "## Summary", "",
        f"- Passed: {snapshot.summary.passed}", f"- Warning: {snapshot.summary.warning}",
        f"- Failed: {snapshot.summary.failed}", f"- Total: {snapshot.summary.total}",
    ]
    lines.extend(_quality_markdown(evidence))
    lines.extend(["", "## Criteria", "", "| Category | Criterion | Status | Value | Threshold |", "|---|---|---|---|---|"])
    ...
    lines.extend(["", "## Role Sign-offs", "", "| Role | Signed Off | Signed Off By | Signed Off At | Note |", "|---|---|---|---|---|"])
    for signoff in snapshot.signoffs:
        signed_at = signoff.signed_off_at.astimezone(timezone.utc).strftime("%Y-%m-%d %H:%M UTC") if signoff.signed_off_at else ""
        lines.append(
...
async def get_release_readiness(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)) -> ReadinessSnapshot:
    _ = current_user
    return await crud.get_readiness_snapshot(db)


@router.post("/signoff", response_model=ReadinessSnapshot)
async def signoff_release_readiness(payload: SignoffRequest, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)) -> ReadinessSnapshot:
...
async def revoke_release_signoff(role: str, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)) -> ReadinessSnapshot:
...
async def request_release_signoff(payload: SignoffRequest, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)) -> Response:
    role = _normalize_role(payload.role)
...
async def export_release_readiness(
    ...
    if format == "md":
        return Response(content=markdown_report, media_type="text/markdown", headers={"Content-Disposition": "attachment; filename=release_readiness_report.md"})
    ...
    summary_data = [["Metric", "Value"], ["Passed", str(snapshot.summary.passed)], ["Warning", str(snapshot.summary.warning)], ["Failed", str(snapshot.summary.failed)], ["Total", str(snapshot.summary.total)]]
    summary_table = Table(summary_data, colWidths=[2.5 * inch, 2.0 * inch])
    summary_table.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, 0), colors.darkblue), ("TEXTCOLOR", (0, 0), (-1, 0), colors.whitesmoke), ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"), ("GRID", (0, 0), (-1, -1), 0.5, colors.black), ("ROWBACKGROUNDS", (0, 1), (-1, -1), [colors.white, colors.lightgrey])]))
    elements.extend([summary_table, Spacer(1, 0.2 * inch), Paragraph(f"Quality KPI Evidence — Last {int(window)} Days", styles["Heading2"])])
    for line in _quality_markdown(evidence)[2:]:
        if line.startswith("### "):
            elements.append(Paragraph(line[4:], styles["Heading3"]))
        elif line.startswith("- "):
            elements.append(Paragraph(line[2:].replace("**", ""), styles["BodyText"]))
    elements.extend([Spacer(1, 0.2 * inch), Paragraph("Criteria", styles["Heading2"])])
    criteria_data = [["Category", "Criterion", "Status", "Value"]] + [[c.category, c.label, c.status, c.value] for c in snapshot.criteria]
    criteria_table = Table(criteria_data, colWidths=[1.2 * inch, 3.0 * inch, 1.0 * inch, 1.4 * inch], repeatRows=1)
    criteria_table.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, 0), colors.grey), ("TEXTCOLOR", (0, 0), (-1, 0), colors.whitesmoke), ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"), ("FONTSIZE", (0, 0), (-1, -1), 8), ("GRID", (0, 0), (-1, -1), 0.5, colors.black), ("ROWBACKGROUNDS", (0, 1), (-1, -1), [colors.white, colors.lightgrey])]))
    elements.extend([criteria_table, Spacer(1, 0.2 * inch), Paragraph("Sign-offs", styles["Heading2"])])
    signoff_data = [["Role", "Signed Off", "By", "At"]]
    for signoff in snapshot.signoffs:
        signed_at = signoff.signed_off_at.astimezone(timezone.utc).strftime("%Y-%m-%d %H:%M UTC") if signoff.signed_off_at else ""
        signoff_data.append([signoff.role, "yes" if signoff.signed_off else "no", signoff.signed_off_by or "", signed_at])
    signoff_table = Table(signoff_data, colWidths=[1.2 * inch, 1.0 * inch, 2.0 * inch, 2.0 * inch], repeatRows=1)
    signoff_table.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, 0), colors.grey), ("TEXTCOLOR", (0, 0), (-1, 0), colors.whitesmoke), ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"), ("FONTSIZE", (0, 0), (-1, -1), 8), ("GRID", (0, 0), (-1, -1), 0.5, colors.black), ("ROWBACKGROUNDS", (0, 1), (-1, -1), [colors.white, colors.lightgrey])]))
    elements.append(signoff_table)
    doc.build(elements)
    return Response(content=buffer.getvalue(), media_type="application/pdf", headers={"Content-Disposition": "attachment; filename=release_readiness_report.pdf"})
