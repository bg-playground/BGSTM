...
class SuggestionConfig(BaseModel):
    ...
    min_confidence_threshold: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Minimum confidence score to create a suggestion"
    )
    
    # Default algorithm to use
    default_algorithm: str = Field(
        default="tfidf",
        description="Default similarity algorithm: 'tfidf', 'keyword', or 'hybrid'"
    )
    
    # TF-IDF specific settings
    tfidf_max_features: Optional[int] = Field(
        default=100,
        description="Maximum number of features for TF-IDF vectorizer"
    )
    
    tfidf_ngram_range: tuple[int, int] = Field(
        default=(1, 2),
        description="N-gram range for TF-IDF (min, max)"
    )
    
    # Keyword matching settings
    keyword_min_word_length: int = Field(
        default=3,
        description="Minimum word length for keyword extraction"
    )
    
    keyword_top_n: int = Field(
        default=10,
    ...
    hybrid_tfidf_weight: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Weight for TF-IDF score in hybrid approach"
    )
    ...
    hybrid_keyword_weight: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Weight for keyword score in hybrid approach"
    )
